...
int main()
{
    ...
    float h = 0.0f, c = 0.0f, f = 0.0f, k = 0.0f, dp = 0.0f, dpf = 0.0f;
	  char buffer[13] = "Temperature:";
	  char buffer1[10] = "Humidity:";
		char tnumber[32];
		char hnumber[32];
    pc.printf("Temperature and Humidity display test begin!\r\n");
...
//            dpf = sensor.CalcdewPointFast(c, h);
      ...
      display.setTextColor(WHITE);
			...
			for (uint8_t i=0; i <4; i++){
				display.write(tnumber[i]);
			}
			display.write('c');
			display.write('\n');
			for (uint8_t i=0; i <10; i++) {
				display.write(buffer1[i]);
			}
			sprintf(hnumber,"%f",h);
			for (uint8_t i=0; i <4; i++){
				display.write(hnumber[i]);
			}
			display.write('%');
			display.display();
			wait(5);
